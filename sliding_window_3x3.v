...
module sliding_window_3x3 #(
    ...
    output reg  [DATA_WIDTH-1:0] w0, w1, w2, w3, w4, w5, w6, w7, w8
);

    // Line buffers - store 2 complete rows
    reg  [DATA_WIDTH-1:0] line0 [0:IMG_WIDTH-1];  // Oldest row
    reg  [DATA_WIDTH-1:0] line1 [0:IMG_WIDTH-1];  // Middle row
    
    // Position tracking
    localparam COL_WIDTH = (IMG_WIDTH <= 1) ? 1 : $clog2(IMG_WIDTH);
    reg [COL_WIDTH-1:0] col;
    reg [15:0] row;
    
    // Window buffers - hold current 3x3 window
    reg [DATA_WIDTH-1:0] win_buf [0:2][0:2];  // [row][col]
